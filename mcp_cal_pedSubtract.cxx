...
void mcp_cal_pedSubtract (const char*);

int main (int argc, char *argv[]) {
	while (*++argv) {
		mcp_cal_pedSubtract(argv[0]);
	}
	return 0;
}

void mcp_cal_pedSubtract (const char *filename) {
	
	printf("\nmcp_cal_pedSubtract started.\n");
    printf(filename);
    
	/*****************************************************************/
	/*** Code to include source code as a string in the ROOT file: ***/
	/*****************************************************************
    FILE *fsrc = fopen("mcp_cal_pedSubtract.cxx", "r");
    char *fbuf;
    struct stat srcbuf;
	
    stat("mcp_cal_pedSubtract.cxx", &srcbuf);
...
*/	
	...
	char *dir = "mcp_cal_pedSubtract"; // results will be placed in this subdirectory of the root file
	char *dir_cycle = "mcp_cal_pedSubtract;1"; // results will be placed in this subdirectory of the root file
...
// RIGHT MCP	
	...
	TH2I *h_R_mcp_map_missingA_2		= new TH2I("h_R_mcp_map_missingA_2", "Right MCP map w/ sum > 400, events where post A was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_R_mcp_map_missingB_2		= new TH2I("h_R_mcp_map_missingB_2", "Right MCP map w/ sum > 400, events where post B was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_R_mcp_map_missingC_2		= new TH2I("h_R_mcp_map_missingC_2", "Right MCP map w/ sum > 400, events where post C was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_R_mcp_map_missingD_2		= new TH2I("h_R_mcp_map_missingD_2", "Right MCP map w/ sum > 400, events where post D was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_R_mcp_map_reconstructed_2	= new TH2I("h_R_mcp_map_reconstructed_2", "Right MCP map w/ sum > 400, single missing posts reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_T_mcp_map_missingA_2		= new TH2I("h_T_mcp_map_missingA_2", "Top MCP map w/ sum > 400, events where post A was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_T_mcp_map_missingB_2		= new TH2I("h_T_mcp_map_missingB_2", "Top MCP map w/ sum > 400, events where post B was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_T_mcp_map_missingC_2		= new TH2I("h_T_mcp_map_missingC_2", "Top MCP map w/ sum > 400, events where post C was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_T_mcp_map_missingD_2		= new TH2I("h_T_mcp_map_missingD_2", "Top MCP map w/ sum > 400, events where post D was reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	TH2I *h_T_mcp_map_reconstructed_2	= new TH2I("h_T_mcp_map_reconstructed_2", "Top MCP map w/ sum > 400, single missing posts reconstructed", mapBins, mapMin, mapMax, mapBins, mapMin, mapMax);
	
	TH1I *h_R_mcpA_reconstruction_diffs	= new TH1I("h_R_mcpA_reconstruction_diffs","(Reconstructed A - Measured A) for events with all 4 posts, Right MCP", 30000, -5000, 15000);
	TH1I *h_R_mcpB_reconstruction_diffs	= new TH1I("h_R_mcpB_reconstruction_diffs","(Reconstructed B - Measured B) for events with all 4 posts, Right MCP", 30000, -5000, 15000);
	TH1I *h_R_mcpC_reconstruction_diffs	= new TH1I("h_R_mcpC_reconstruction_diffs","(Reconstructed C - Measured C) for events with all 4 posts, Right MCP", 30000, -5000, 15000);
	TH1I *h_R_mcpD_reconstruction_diffs	= new TH1I("h_R_mcpD_reconstruction_diffs","(Reconstructed D - Measured D) for events with all 4 posts, Right MCP", 30000, -5000, 15000);
	TH1I *h_T_mcpA_reconstruction_diffs	= new TH1I("h_T_mcpA_reconstruction_diffs","(Reconstructed A - Measured A) for events with all 4 posts, Top MCP", 30000, -5000, 15000);
	TH1I *h_T_mcpB_reconstruction_diffs	= new TH1I("h_T_mcpB_reconstruction_diffs","(Reconstructed B - Measured B) for events with all 4 posts, Top MCP", 30000, -5000, 15000);
	TH1I *h_T_mcpC_reconstruction_diffs	= new TH1I("h_T_mcpC_reconstruction_diffs","(Reconstructed C - Measured C) for events with all 4 posts, Top MCP", 30000, -5000, 15000);
	TH1I *h_T_mcpD_reconstruction_diffs	= new TH1I("h_T_mcpD_reconstruction_diffs","(Reconstructed D - Measured D) for events with all 4 posts, Top MCP", 30000, -5000, 15000);
	
	TH1I *h_R_mcpA_normed_reconstruction_diffs	= new TH1I("h_R_mcpA_normed_reconstruction_diffs","(Reconstructed A - Measured A)/(Measured A) for events with all 4 posts, Right MCP", 10000, -1, 3);
	TH1I *h_R_mcpB_normed_reconstruction_diffs	= new TH1I("h_R_mcpB_normed_reconstruction_diffs","(Reconstructed B - Measured B)/(Measured B) for events with all 4 posts, Right MCP", 10000, -1, 3);
	TH1I *h_R_mcpC_normed_reconstruction_diffs	= new TH1I("h_R_mcpC_normed_reconstruction_diffs","(Reconstructed C - Measured C)/(Measured C) for events with all 4 posts, Right MCP", 10000, -1, 3);
	TH1I *h_R_mcpD_normed_reconstruction_diffs	= new TH1I("h_R_mcpD_normed_reconstruction_diffs","(Reconstructed D - Measured D)/(Measured D) for events with all 4 posts, Right MCP", 10000, -1, 3);
	TH1I *h_T_mcpA_normed_reconstruction_diffs	= new TH1I("h_T_mcpA_normed_reconstruction_diffs","(Reconstructed A - Measured A)/(Measured A) for events with all 4 posts, Top MCP", 10000, -1, 3);
	TH1I *h_T_mcpB_normed_reconstruction_diffs	= new TH1I("h_T_mcpB_normed_reconstruction_diffs","(Reconstructed B - Measured B)/(Measured B) for events with all 4 posts, Top MCP", 10000, -1, 3);
	TH1I *h_T_mcpC_normed_reconstruction_diffs	= new TH1I("h_T_mcpC_normed_reconstruction_diffs","(Reconstructed C - Measured C)/(Measured C) for events with all 4 posts, Top MCP", 10000, -1, 3);
	TH1I *h_T_mcpD_normed_reconstruction_diffs	= new TH1I("h_T_mcpD_normed_reconstruction_diffs","(Reconstructed D - Measured D)/(Measured D) for events with all 4 posts, Top MCP", 10000, -1, 3);
...
// TOP MCP		
		...
		if (tSum>400) {
			...
			h_T_mcp_map_reconstructed_2->Fill(tX,tY);
		}
...
// RIGHT MCP		
		...
		if (rSum>400) {
			...
			h_R_mcp_map_reconstructed_2->Fill(rX,rY);
		}
...
// RIGHT MCP
// A post missing:
		if (rB+rC+rD > 400) { // other 3 posts are present if they pass this test
			rA_calc = rB*rD/rC;
			if (printReconstructionMessage) printf("a_R_mcpA: Data value = %6f; Reconstructed value = %6f.\n", rA, rA_calc);
			if (rA < 0) { // this post is missing
...
//				rX_mm = scale * ((rX-x0)*TMath::Cos(theta) - (rY-y0)*TMath::Sin(theta));
//				rY_mm = scale * ((rX-x0)*TMath::Sin(theta) + (rY-y0)*TMath::Cos(theta));
				h_R_mcp_map_missingA_2->Fill(rX,rY);
				h_R_mcp_map_reconstructed_2->Fill(rX,rY);// this is also filled under if (rSum > 400), above
			}
...
// B post missing:
		if (rA+rC+rD > 400) { // other 3 posts are present if they pass this test
			rB_calc = rA*rC/rD;
			if (printReconstructionMessage) printf("a_R_mcpB: Data value = %6f; Reconstructed value = %6f.\n", rB, rB_calc);
			if (rB < 0) { // this post is missing
...
//				rX_mm = scale * ((rX-x0)*TMath::Cos(theta) - (rY-y0)*TMath::Sin(theta));
//				rY_mm = scale * ((rX-x0)*TMath::Sin(theta) + (rY-y0)*TMath::Cos(theta));
				h_R_mcp_map_missingB_2->Fill(rX,rY);
				h_R_mcp_map_reconstructed_2->Fill(rX,rY);// this is also filled under if (rSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// C post missing:
		if (rA+rB+rD > 400) { // other 3 posts are present if they pass this test
			rC_calc = rB*rD/rA;
			if (printReconstructionMessage) printf("a_R_mcpC: Data value = %6f; Reconstructed value = %6f.\n", rC, rC_calc);
			if (rC < 0) { // this post is missing
...
//				rX_mm = scale * ((rX-x0)*TMath::Cos(theta) - (rY-y0)*TMath::Sin(theta));
//				rY_mm = scale * ((rX-x0)*TMath::Sin(theta) + (rY-y0)*TMath::Cos(theta));
				h_R_mcp_map_missingC_2->Fill(rX,rY);
				h_R_mcp_map_reconstructed_2->Fill(rX,rY);// this is also filled under if (rSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// D post missing:
		if (rA+rB+rC > 400) { // other 3 posts are present if they pass this test
			rD_calc = rA*rC/rB;
			if (printReconstructionMessage) printf("a_R_mcpD: Data value = %6f; Reconstructed value = %6f.\n", rD, rD_calc);
			if (rD < 0) { // this post is missing
...
//				rX_mm = scale * ((rX-x0)*TMath::Cos(theta) - (rY-y0)*TMath::Sin(theta));
//				rY_mm = scale * ((rX-x0)*TMath::Sin(theta) + (rY-y0)*TMath::Cos(theta));
				h_R_mcp_map_missingD_2->Fill(rX,rY);
				h_R_mcp_map_reconstructed_2->Fill(rX,rY);// this is also filled under if (rSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// TOP MCP
// A post missing:
		if (tB+tC+tD > 400) { // other 3 posts are present if they pass this test
			tA_calc = tB*tD/tC;
			if (printReconstructionMessage) printf("a_T_mcpA: Data value = %6f; Reconstructed value = %6f.\n", tA, tA_calc);
			if (tA < 0) { // this post is missing
...
//				tY_mm = scale * ((tX-x0)*TMath::Sin(theta) + (tY-y0)*TMath::Cos(theta));
				h_T_mcp_map_missingA_2->Fill(tX,tY);
				h_T_mcp_map_reconstructed_2->Fill(tX,tY);// this is also filled under if (tSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// B post missing:
		if (tA+tC+tD > 400) { // other 3 posts are present if they pass this test
			tB_calc = tA*tC/tD;
			if (printReconstructionMessage) printf("a_T_mcpB: Data value = %6f; Reconstructed value = %6f.\n", tB, tB_calc);
			if (tB < 0) { // this post is missing
...
//				tX_mm = scale * ((tX-x0)*TMath::Cos(theta) - (tY-y0)*TMath::Sin(theta));
//				tY_mm = scale * ((tX-x0)*TMath::Sin(theta) + (tY-y0)*TMath::Cos(theta));
				h_T_mcp_map_missingB_2->Fill(tX,tY);
				h_T_mcp_map_reconstructed_2->Fill(tX,tY);// this is also filled under if (tSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// C post missing:
		if (tA+tB+tD > 400) { // other 3 posts are present if they pass this test
			tC_calc = tB*tD/tA;
			if (printReconstructionMessage) printf("a_T_mcpC: Data value = %6f; Reconstructed value = %6f.\n", tC, tC_calc);
			if (tC < 0) { // this post is missing
...
//				tX_mm = scale * ((tX-x0)*TMath::Cos(theta) - (tY-y0)*TMath::Sin(theta));
//				tY_mm = scale * ((tX-x0)*TMath::Sin(theta) + (tY-y0)*TMath::Cos(theta));
				h_T_mcp_map_missingC_2->Fill(tX,tY);
				h_T_mcp_map_reconstructed_2->Fill(tX,tY);// this is also filled under if (tSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
// D post missing:
		if (tA+tB+tC > 400) { // other 3 posts are present if they pass this test
			tD_calc = tA*tC/tB;
			if (printReconstructionMessage) printf("a_T_mcpD: Data value = %6f; Reconstructed value = %6f.\n", tD, tD_calc);
			if (tD < 0) { // this post is missing
...
//				tX_mm = scale * ((tX-x0)*TMath::Cos(theta) - (tY-y0)*TMath::Sin(theta));
//				tY_mm = scale * ((tX-x0)*TMath::Sin(theta) + (tY-y0)*TMath::Cos(theta));
				h_T_mcp_map_missingD_2->Fill(tX,tY);
				h_T_mcp_map_reconstructed_2->Fill(tX,tY);// this is also filled under if (tSum > 400), above
			}
			else { // all 4 posts are present and we can compare measured versus "reconstructed" value
...
//	gDirectory->WriteTObject(fstr);
	...
	gDirectory->WriteTObject(h_R_mcp_map_missingA_2);
	gDirectory->WriteTObject(h_R_mcp_map_missingB_2);
	gDirectory->WriteTObject(h_R_mcp_map_missingC_2);
	gDirectory->WriteTObject(h_R_mcp_map_missingD_2);
	gDirectory->WriteTObject(h_R_mcp_map_reconstructed_2);
	
	gDirectory->WriteTObject(h_T_mcp_map_missingA_2);
	gDirectory->WriteTObject(h_T_mcp_map_missingB_2);
	gDirectory->WriteTObject(h_T_mcp_map_missingC_2);
	gDirectory->WriteTObject(h_T_mcp_map_missingD_2);
	gDirectory->WriteTObject(h_T_mcp_map_reconstructed_2);
