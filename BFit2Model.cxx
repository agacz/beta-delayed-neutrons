...
// precomputes dozens of special values that are used throughout the evalations
// of the component functions. These values live in the global namespace and are
// declared above main() in BFit2.cxx.
//
// A few of these values depend on the variable (t) and used many tvars, so yAll
// and stores them for re-use. So far these are just the injection number nCap
// and all the exp decay factors.
//
// But most of the values are parameter-dependent and independent of t, so computing
// them for every t and in every component was a huge waste of tvar. The fitting
// was just too slow. (Also made for ugly code, but made it easy to build up the 
// model one function at a tvar.) So the yAll function detects a change in the parameters
...
// means that the definitions of values are not near their use. I you are reading this
...
//////////////////////////////////////////////////////////////////////////
// Global variables -- see BFit2.cxx for more info
//////////////////////////////////////////////////////////////////////////
// Global variables that are constant throughout the fit
	extern Int_t nPars, nCapMax;
	extern Double_t iota, tCap, tBac, tCyc, t1, t2, t3;
	extern Double_t eU1tCyc, eU2tCyc, eU3tCyc;
	extern BDNCase_t stBDNCases[FILE_ROWS_BDN];
	extern Int_t iBDNCaseIndex;
...
// Update shared tvar-dependent variables
	tvar	= t[0];
...
// When parameters change:
	...
	// Update parameter-dependent variables
		...
		ampY3fromY2_ST1		= a[r1]*(tCap/t2)*tU1*tU2*tU3/ThetaU/t1*(a[gammaT1]+iota)/(a[gammaT1]-a[gammaU1]+iota)*(tU1-tT1)/tU1/(a[gammaT1]/1000.0)/ThetaY;
...
//		Double_t tCycArg[1] = {tCyc};
	...
	// Initial values of populations
		...
		//////////////////////////////////////////
//		Y20 = Ycap(2,a,tCyc) / (1-eU2tCyc);
//		Y30 = Ycap(3,a,tCyc) / (1-eU3tCyc);
//		u10 = V1(tCycArg,a) + W1(tCycArg,a) + Z1(tCycArg,a) / (1-eU1tCyc);
//		y20 = Y2Trapping(tCycArg,a,N) / (1-eU2tCyc);
		...
		Y30 = ( Ycap(3,a,tCyc)
			...
			+ U10 * tU1/t1 * tU2/t2 * tU3/ThetaU * ( tU1 * (tU3-tU2) * eU1tCyc - tU2 * (tU3-tU1) * eU2tCyc + tU3 * (tU2-tU1) * eU3tCyc ) )
			/ ( 1 - eU3tCyc );
...
//////////////////////////////////////////////////
// END OF PARAMETER-DEPENDENT VARIABLES	
//////////////////////////////////////////////////
	
// Evaluate ion populations -- last thing before 
/*	T1val	= Ttot(1,a,tvar);
	T2val	= Ttot(2,a,tvar);
	T3val	= Ttot(3,a,tvar);
	V1val	= Vtot(1,a,tvar);
...
//	return a[nCyc]*a[dt]*(a[dc] + a[epsT]*(Ttot(1,a,tvar)/t1 + Ttot(2,a,tvar)/t2;
}
/////////////////////////////////////////////////////////////////////
...
Double_t BFitNamespace::yT1 (Double_t *t, Double_t *a) {
	printf("yT1: t[0] = %f.  ",t[0]);
	return yT(1,a,t[0]); }
Double_t BFitNamespace::yT2 (Double_t *t, Double_t *a) { return yT(2,a,t[0]); }
Double_t BFitNamespace::yT3 (Double_t *t, Double_t *a) { return yT(3,a,t[0]); }
/////////////////////////////////////////////////////////////////////
...
Double_t BFitNamespace::yT (Int_t i, Double_t *a, Double_t tvar) {
	...
	if (i==2) {
		f = a[nCyc]*a[dt]*a[epsT]*Ttot(2,a,tvar)/t2;
		printf("yT: tvar = %f.",tvar);
	}
...
Double_t BFitNamespace::yU (Int_t i, Double_t *a, Double_t tvar) {
	...
	static Double_t f;
	f = 0.0;
	if (i==1) f = a[nCyc] *a[dt] * ( a[epsV]*Vtot(1,a,tvar) + a[epsW]*Wtot(1,a,tvar) + a[epsZ]*Ztot(1,a,tvar) + a[epsX]*Xtot(1,a,tvar) + a[epsY]*Ytot(1,a,tvar) ) / t1;
...
Double_t BFitNamespace::Ttot (Int_t i, Double_t *a, Double_t tvar) {
	...
	using namespace TMath;
	extern Int_t nCap;
	extern Double_t tBac, tCyc, tT1, tT2, tT3;
	extern Double_t ampT1, ampT2, ampT3;
	extern Double_t eT1nCap, eT2nCap, eT3nCap;
	static Double_t f;
	f = 0.0;
	if (tBac <= tvar && tvar <= tCyc) {
		if (i==1) f = ampT1 * SigmaT(a[rho],tT1,nCap) * eT1nCap;
		if (i==2) {
			f = ampT2 * SigmaT(a[rho],tT2,nCap) * eT2nCap;
			printf("Ttot: tvar = %f.  eT2nCap = %f.\n", tvar, eT2nCap);
		}
		if (i==3) f = ampT3 * SigmaT(a[rho],tT3,nCap) * eT3nCap;
	}
	return f;
}
...
Double_t BFitNamespace::Vcap (Int_t i, Double_t *a, Double_t tvar) {
	...
	using namespace TMath;
	extern Int_t nCap;
	extern Double_t tBac, tCyc, tU1, tU2, tU3;
	extern Double_t ampV1, ampV2, ampV3;
	extern Double_t eU1nCap, eU2nCap, eU3nCap;
	static Double_t f;
	f = 0.0;
	if (i==1) f = ampV1 * SigmaT(1,tU1,nCap) * eU1nCap;
	if (i==2) f = ampV2 * SigmaT(1,tU2,nCap) * eU2nCap;
	if (i==3) f = ampV3 * SigmaT(1,tU3,nCap) * eU3nCap;
	return f;
}
...
Double_t BFitNamespace::Wcap (Int_t i, Double_t *a, Double_t tvar) {
	...
	using namespace TMath;
	extern Int_t nCap;
	extern Double_t tCap, tBac, tCyc, tT1, tT2, tT3, tU1, tU2, tU3;
	extern Double_t ampW1, ampW2, ampW3;
	extern Double_t eU1nCap, eU2nCap, eU3nCap;
	static Double_t f;
	f = 0.0;
	if (i==1) f += ampW1 * SigmaW(a[rho],tT1,tU1,nCap) * eU1nCap;
	if (i==2) f += ampW2 * SigmaW(a[rho],tT2,tU2,nCap) * eU2nCap;
	if (i==3) f += ampW3 * SigmaW(a[rho],tT3,tU3,nCap) * eU3nCap;
	return f;
}
...
Double_t BFitNamespace::Zcap (Int_t i, Double_t *a, Double_t tvar) {
	...
	static Double_t f;
	f = 0.0;
	if (i==1) f += ampZ1 * ( (SigmaZ(a[rho],tT1,tU1,nCap)+SigmaT(a[rho],tT1,nCap))*eU1nCap - SigmaT(a[rho],tT1,nCap)*eT1nCap );
	if (i==2) f += ampZ2 * ( (SigmaZ(a[rho],tT2,tU2,nCap)+SigmaT(a[rho],tT2,nCap))*eU2nCap - SigmaT(a[rho],tT2,nCap)*eT2nCap );
	if (i==3) f += ampZ3 * ( (SigmaZ(a[rho],tT3,tU3,nCap)+SigmaT(a[rho],tT3,nCap))*eU3nCap - SigmaT(a[rho],tT3,nCap)*eT3nCap );
	return f;
}
...
Double_t BFitNamespace::Xcap (Int_t i, Double_t *a, Double_t tvar) {
	...
	using namespace TMath;
	extern Int_t nCap;
	extern Double_t tCap, tBac, tCyc, tT1, tT2, tU1, tU2, tU3;
	extern Double_t ampX2, ampX3;
	extern Double_t eT1nCap, eT2nCap, eU2nCap, eU3nCap;
	static Double_t f;
	f = 0.0;
	if (i==2) f += ampX2 * ( ( SigmaZ(a[rho],tT1,tU2,nCap) + SigmaT(a[rho],tT1,nCap) ) * eU2nCap - SigmaT(a[rho],tT1,nCap) * eT1nCap );
	if (i==3) f += ampX3 * ( ( SigmaZ(a[rho],tT2,tU3,nCap) + SigmaT(a[rho],tT2,nCap) ) * eU3nCap - SigmaT(a[rho],tT2,nCap) * eT2nCap );
	return f;
}
Double_t BFitNamespace::Xtot (Int_t i, Double_t *a, Double_t tvar) {
	extern Double_t tBac, tCyc, tU2, tU3, X20, X30;
	extern Double_t eU20, eU30;
	static Double_t f;
	f = 0.0; //catch bad values of tvar
	if (0 <= tvar && tvar < tCyc) {
		if (i==2) f += X20 * eU20;
...
Double_t BFitNamespace::Ytot (Int_t i, Double_t *a, Double_t tvar) {
	...
	f = 0.0; //catch bad values of tvar
	if (0 <= tvar && tvar < tCyc) {
		if (i==2) f += Y2Background(U10,Y20);
		if (i==3) f += Y3Background(U10,U20,Y30);
	}
	if (tBac <= tvar && tvar <= tCyc)
...
Double_t BFitNamespace::Y2Background (Double_t u10, Double_t u20) {
	using namespace TMath;
	extern Double_t t1, tU1, tU2;
	extern Double_t eU10, eU20;
	return u20 * eU20 + u10 * tU1/t1 * tU2/(tU2-tU1) * ( eU20 - eU10 );
}
Double_t BFitNamespace::Y3Background (Double_t u10, Double_t u20, Double_t u30) {
	using namespace TMath;
	extern Double_t tBac, tCyc, t1, t2;
	extern Double_t tU1, tU2, tU3, ThetaU;
	extern Double_t eU10, eU20, eU30;
	return u30 * eU30
			+ u20 * tU2/t2 * tU3/(tU3-tU2) * ( eU30 - eU20 )
			+ u10 * tU1/t1 * tU2/t2 * tU3/ThetaU * ( tU1*(tU3-tU2)*eU10 - tU2*(tU3-tU1)*eU20 + tU3*(tU2-tU1)*eU30 );
}
...
Double_t BFitNamespace::Y2InitialValue (Double_t tvar, Double_t *a, Double_t t0, Double_t y0) {
	...
	f = 0.0;
	tk = tvar-t0;
//	A = ampY2ptA * SigmaT(a[rho],tT1,1) * (cT1*Exp(-tk/tT1) - cU1*Exp(-tk/tU1) + cU2*Exp(-tk/tU2));
...
//		+ ampY3fromY2_SW11 * SW11_1cap * ( cYU1*eU1 - cYU2*eU2 + cYU3*eU3 );
//		+ ampY3fromY2_SZ11 * SZ11_1cap * ( cYU1*eU1 - cYU2*eU2 + cYU3*eU3 ); // feeding from Y2
	V = ampY3fromV2 * ST2_1cap  * (eU3 - eU2); // feeding from V2
	W = ampY3fromW2 * SW22_1cap * (eU3 - eU2); // feeding from W2
	Z = ampY3fromZ2 * ( (ST2_1cap/cZU2) * ( cZT2 * eT2 - cZU2 * eU2 + cZU3 * eU3 ) );// + SZ22_1cap*(eU3-eU2) ); // feeding from Z2
	X = ampY3fromX2 * ( (ST1_1cap/cXU2) * ( cXT1 * eT1 - cXU2 * eU2 + cXU3 * eU3 ) );// + SZ12_1cap*(eU3-eU2) ); // feeding from X2
...
Double_t BFitNamespace::Y3Trapping (Double_t *t, Double_t *a, Int_t n) {
	using namespace BFitNamespace;
	extern Double_t tCap, tBac, tCyc;
	static Double_t tk, f;
	static Int_t k;
	if (t[0] < tBac || t[0] > tCyc) return 0.0;
	else {
		f = 0.0;
		for (k=1; k<=n; k++) f += Y3InitialValue(t, a, tBac+(k-1)*tCap, 0.0);
		return f;
	}
}
...
Double_t BFitNamespace::T1 (Double_t *t, Double_t *a) {
	...
	if (tBac <= t[0] && t[0] <= tCyc)
	{
		tT1 = 1.0 / ( 1.0/t1 + a[gammaT1]/1000.0 ); // net variable lifetvar (1/e) in ms
		n = Ceil((t[0]-tBac)/tCap);
		f = a[p] * a[r1] * tCap * SigmaT(a[rho],tT1,nCap) * Exp(-(t[0]-tBac-(nCap-1)*tCap)/tT1);
	}
	return f;
}
...
Double_t BFitNamespace::T2 (Double_t *t, Double_t *a) {
	...
	if (tBac <= t[0] && t[0] <= tCyc)
	{
		tT2 = 1.0 / ( 1.0/t2 + a[gammaT2]/1000.0 ); // net variable lifetvar (1/e) in ms
		n = Ceil((t[0]-tBac)/tCap);
		f = a[p] * a[r2] * tCap * SigmaT(a[rho],tT2,nCap) * Exp(-(t[0]-tBac-(nCap-1)*tCap)/tT2);
	}
	return f;
}
...
Double_t BFitNamespace::T3 (Double_t *t, Double_t *a) {
	...
	if (tBac <= t[0] && t[0] <= tCyc)
	{
		...
		tT3 = 1.0 / ( 1.0/t3 + a[gammaT3]/1000.0 ); // net variable lifetvar (1/e) in ms
		n = Ceil((t[0]-tBac)/tCap);
		f = a[p] * a[r3] * tCap * SigmaT(a[rho],tT3,nCap) * Exp(-(t[0]-tBac-(nCap-1)*tCap)/tT3);
	}
	return f;
}
*/
